...
class Run(object):
  ...
  def Run(self, args):
    ...
    self._AppendToEnvPath('PYTHONPATH', path.join(rpa_root, 'libraries'))
    self._AppendToEnvPath('PYTHONPATH', path.join(rpa_root, 'modules'))
    self._AppendToEnvPath('PATH', '.')
    os.environ['LD_LIBRARY_PATH'] = path.join(rpa_root, 'libraries')
