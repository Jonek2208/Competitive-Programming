...
class Build(object):
  ...
  def _CBuildCommands(self, args):
    ...
    rpa_root = path.join(path.dirname(path.realpath(__file__)), '..')
    include_dir = path.join(rpa_root, 'includes')
    local_zeus_path = path.join(rpa_root, 'libraries', 'zeus_local.c')
    message_path = path.join(rpa_root, 'libraries', 'message_internal.c')
  ...
  def _CcBuildCommands(self, args):
    ...
    c_sources = (path.join(rpa_root, 'libraries', 'zeus_local.c'),
                 path.join(rpa_root, 'libraries', 'message_internal.c'),
                )
  ...
  def _BuildPythonObjectFileCommand(self, c_source, output):
    ...
    return (
        ...
        '-I' + path.join(rpa_root, 'includes'),
        path.join(rpa_root, 'libraries', c_source),
        '-o', path.join(rpa_root, 'libraries', output),
    )
  ...
  def _PyBuildCommands(self, unused_args):
    ...
    object_files = tuple(
        path.join(rpa_root, 'libraries', item + '.o')
        for item in self._MESSAGE_SO_PYTHON_INGREDIENTS
    )
    ...
    link_object_files = (
        ('ld', '-shared',) +
        object_files +
        ('-o', path.join(rpa_root, 'libraries', '_message.so',))
    )
  ...
  def _JavaBuildCommands(self, args):
    ...
    rpa_root = path.join(path.dirname(path.realpath(__file__)), '..')
    message_dir = path.join(rpa_root, 'libraries')
    ...
    build_class_file_command = (
        ...
        path.join(rpa_root, 'libraries', 'Wrapper.java'),
        args['source'],
        path.join(rpa_root, 'libraries', 'message.java'),
        path.join(rpa_root, 'libraries', 'messageJNI.java'),
